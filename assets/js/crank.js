  ...
  $.widget('ui.rotatable', $.ui.mouse, {
    ...
    _calculateElementCenter: function() {
      ...
      if (this.element.css('transform-origin') !== undefined) {
        var originPx = this.element.css('transform-origin').match(
          /([\d.]+)px +([\d.]+)px/)
        if (originPx != null) {
          return {
    ...
    _create: function() {
      ...
      if (!this.options.handle) {
        ...
        if (this.options.handleOffset.top !== 0 || this.options.handleOffset.left !==
          0) {
    ...
    _isRotationCenterSet: function() {
      return (this.options.rotationCenterOffset.top !== 0 || this.options.rotationCenterOffset
        .left !== 0)
    },
    ...
    _performRotation: function(angle) {
      if (this._isRotationCenterSet()) {
        this.element.css('transform-origin', this.options.rotationCenterOffset.left +
          'px ' + this.options.rotationCenterOffset.top + 'px')
        this.element.css('-ms-transform-origin', this.options.rotationCenterOffset.left +
          'px ' + this.options.rotationCenterOffset.top + 'px') /* IE 9 */
        this.element.css('-webkit-transform-origin', this.options.rotationCenterOffset
          .left + 'px ' + this.options.rotationCenterOffset + 'px') /* Chrome, Safari, Opera */
      }
...
$(document).ready(function() {
  ...
  var counter = 5;
  var handled = false;
  $("#volume").height(counter.toString() + "0%");
  var params = {
    ...
    rotate: function(event, ui) {
      ...
      if (degrees > 345 && last < 15) {
        counter--;
        if (counter < 0) {
          counter = 0;
        }
      } else if (degrees < 15 && last > 345) {
        counter++;
        if (counter > 10) {
          counter = 10;
        }
